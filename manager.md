...
- 복합 키워드(예: "대학 입시 재수")가 검색+dedup 이후에도 `per_keyword_count`에 못 미치면, `worker/src/expansion/keyword_expansion.py`가 자동으로 구성 단어별 재검색을 시도하고(`main.py:_expand_thin_keywords`), 그래도 부족하면 `cfg.llm.enabled`일 때만 LLM에게 연관 검색어를 1회 물어 추가 검색함. 단일어 키워드나 이미 충분한 키워드는 추가 API/LLM 호출이 전혀 발생하지 않음(비용 절약 원칙 유지). 힌트가 등록된 키워드는 확장된 후보에도 관련성 필터가 재적용되어 동음이의어 오염을 막음.
...
- **자동 완화(2026-07-15~)**: 여러 단어로 이뤄진 복합 키워드는 부족할 때 자동으로 구성 단어/연관어 확장 검색이 시도됨(6절 참고). 로그에서 `키워드 확장: keyword=... 기존=N건, ... M건 추가` 라인이 있는지로 확장이 실행됐는지 확인 가능 — 그 확장을 거치고도 적었다면 그날 실제 관련 기사 자체가 부족했던 것.
