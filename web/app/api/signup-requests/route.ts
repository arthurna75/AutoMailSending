...
export async function POST(request: Request) {
  ...
  // "Allow new users to sign up"이 꺼져 있으면 GoTrue가 400 대신 signup_disabled류 에러로
  // "Signups not allowed for otp"를 돌려주는데, 이 프로젝트는 그 설정을 의도적으로 꺼두므로
  // 신규 이메일은 사실상 항상 이 경로로 들어온다. error.code가 항상 채워지는 게 아니라서
  // 메시지 텍스트도 함께 확인한다.
  const isUnknownUserError =
    error.status === 400 ||
    error.code === "signup_disabled" ||
    /not allowed for otp/i.test(error.message);

  if (isUnknownUserError) {
